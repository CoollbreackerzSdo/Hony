...
/// </summary>
public sealed class PaginationCategoryHandler(IUnitOfWord word) : IHandler<PaginationCommandHandler, ImmutableList<string>>
{
    /// <summary>
    ...
    public Result<ImmutableList<string>> Handle(PaginationCommandHandler command)
        => word.CategoryRepository.ImmutablePagination(command, x => x.Name, x => x.Name);
}